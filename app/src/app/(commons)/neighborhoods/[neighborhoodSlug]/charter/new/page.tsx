...
import { draftSectionAction } from './action';

type RouteParams = { neighborhoodSlug: string };
type SearchParams = { error?: string };

export default async function NewCharterSectionPage({
  params,
  searchParams,
}: {
  ...
  if (!session.ok) redirect(`/login?next=/neighborhoods/${neighborhoodSlug}/charter/new`);
  ...
  if (result.membership.role !== 'steward') {
    redirect(`/neighborhoods/${neighborhoodSlug}/charter`);
  }

  const back = `/neighborhoods/${neighborhoodSlug}/charter` as Route;
    ...
    >
      <header className="mb-6 border-b border-[color:var(--color-rule)] pb-5">
        ...
        <p className="mt-1 text-(length:--text-small) text-[color:var(--color-muted)]">
          New sections start as drafts. A steward can ratify when the neighborhood agrees.
        </p>
      </header>
      ...
      {error ? (
        <div className="mb-5 rounded border border-red-200 bg-red-50 px-4 py-3 text-(length:--text-small) text-red-700">
          {error === 'not_steward'
            ? 'Only stewards can draft charter sections.'
            : error === 'invalid'
      ...
      <form action={draftSectionAction} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            required
            maxLength={64}
            placeholder="e.g. resource_guidelines"
            pattern="[a-z0-9_]+"
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <input
            ...
            required
            maxLength={120}
            placeholder="e.g. Resource Guidelines"
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <textarea
            ...
            required
            maxLength={5000}
            placeholder="Write the section text in plain language…"
