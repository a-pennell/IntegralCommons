...
- **CommonGround** is the governance engine — the deliberation framework, constitutional principles, decision methods, and Civic Memory infrastructure.
- **Local Commons** runs CommonGround configured for neighborhood context — lighter quorum thresholds, faster deliberation windows, place-based scoping.
- **MCS** runs a different configuration of the same governance architecture at municipal scale — longer windows, formal decision authority contracts, cross-jurisdiction interoperability.
...
| | CommonGround | MCS |
|---|---|---|
| **Scale** | Any group (5–5,000 members) | Cities, municipalities, civic bodies |
| **Primary users** | Communities, co-ops, land trusts, neighborhoods via Local Commons | Municipal governments, civil society, civic technologists |
| **Decision authority** | Defined by the group's governance profile | Defined by the Decision Authority Contract (DAC) — legal and institutional |
| **Core constraint** | Constitutional principles (Tier 1 inviolables) | Constitutional principles + institutional legal requirements |
| **Deliberation window** | 7 days minimum (configurable) | Longer (civic processes often 30–90 days) |
| **Quorum** | % of affected members | % of affected population (demographic tracking) |
| **Output** | Decision Record (internal institutional memory) | Decision Record + public response to the DAC commitment |
| **Legitimacy source** | Group consent and constitutional process | Group consent + legal/democratic authority of the institution |
...
|---|---|---|
| **Revocability** | Any facilitation delegation can be recalled; no permanent authority | Any Decision Authority Contract can be terminated; no permanent civic mandate |
| **Due Process** | Members subject to removal get transparent process | Residents affected by decisions get structured deliberation before outcome |
| **Commons Protection** | No Decision may privatize neighborhood shared infrastructure | No Civic Process may privatize public infrastructure or restrict civic participation rights |
...
- CommonGround → the governance module as a standalone product
- Local Commons governance → CommonGround configured for neighborhood context
- MCS → a different configuration of the same architecture for civic/municipal context
...
- Quorum is scoped to **affected members**, not the full neighborhood. A decision about the community garden involves garden participants, not every neighbor.
- Default deliberation window: **7 days**
- Default decision method: **consent** (objections must be paramount — reasoned claims of harm, not preferences)
- Facilitation: defaults to the opener; consent check + sortition fallback
- Civic Memory: neighborhood-scoped, full export in Markdown/JSON
- Emergency actions: Steward-level, 72-hour ratification window

### CommonGround (group profile)

- Configurable for any group from 5 to several thousand
- Quorum: configurable by the group's governance profile
- Decision methods: consent by default, but quadratic voting, sortition, liquid democracy available as Phase 2 modules
- Facilitation: as per group's profile
- Civic Memory: group-scoped, full export
- Governance profiles: co-op, board, Indigenous governance, consensus — community-contributed templates

### MCS (civic/municipal profile)

- Quorum: **demographic representativeness tracking** against the affected population, not just % of participants
- Decision Authority Contract: required before any process opens. Defines what decision the process informs and how the output will be used.
- Deliberation window: 30–90 days typical; longer for consequential decisions
- Decision methods: configured per process type (participatory budgeting, land use, citizens' assembly)
- Civic Memory: public record of all processes and outcomes; institutional response to DAC commitments
- Simulation layer: policy trade-off modeling with uncertainty quantification (Phase 2)
- Accountability layer: tracks whether Decision Authority Contracts were honored; flags violations publicly

---

## Interoperability in Practice

**A neighborhood's governance history informs a city's decision:**
A neighborhood opens a governance Decision about a proposed development on their street. They run the 5-phase protocol. The outcome, with Decision Record, is submitted to the city's MCS Civic Process for the rezoning application. The city must acknowledge the input in its outcome record.

**A city's governance profile shapes a neighborhood's defaults:**
A city using MCS for participatory budgeting creates a community-contributed governance profile template for neighborhood sub-processes. Local Commons neighborhoods in that city can adopt the template to align their deliberation format with the city's process requirements.

**Constitutional principles cascade:**
The six Tier 1 inviolable principles apply at every scale. A neighborhood cannot adopt a governance profile that violates Forkability. A city cannot run an MCS Civic Process that violates Deliberation. The constitutional layer is not configurable at either scale.
