...
2. **Boundary-crossing externalities:** The child's decision creates effects that escape the child's boundary and affect sibling holons or the parent's commons. A co-op that dumps waste into the shared watershed has created an externality that the bioregional body must address.
...
### Principle 7 — Bounded Referendum Right
...
### Principle 9 — Deliberation First
...
- **Awareness Quorum** — a percentage of affected members must have viewed the issue before it can move to decision
- **Participation Quorum** — a percentage must have contributed a perspective or explicitly stood aside
- **Decision Quorum** — determined by the decision method in use
...
1. **Notification** — The non-compliance is surfaced. The member is informed and given opportunity to comply voluntarily.
2. **Deliberation** — The group deliberates on the non-compliance. This step is critical because non-compliance may reveal a flaw in the decision itself. If the decision was wrong, the appropriate response is to amend the decision, not to enforce compliance with a bad rule. Deliberation distinguishes between defiance and dissent-through-action.
3. **Proportionate sanction** — Sanctions defined by the group's policy, proportionate to the harm caused by the non-compliance. Loss of specific privileges, temporary restrictions, required mediation, or restorative obligations. Sanctions must be proportionate to the harm, not to the defiance. The purpose of compliance is the health of the commons, not the assertion of authority.
4. **Removal** — For persistent, deliberate non-compliance that harms the commons, removal proceedings under Due Process (Principle 2). This is the final step, not the first response.
...
2. **Between Tier 1 principles,** Revocability (1), Due Process (2), Forkability (4), and Holonic Nesting (5) are co-equal. Conflicts between them are resolved through deliberation with special attention to all affected principles.
...
- The software enforces Tier 1 principles in code — including across nested instances in holonic structures (inter-level Tier 1 compliance checks)
...
*This constitution was developed through structured adversarial deliberation and holonic governance analysis — itself an application of the principles it describes.*
